...
public class SslPerfTest implements Serializable
{
    ...
    public void testSslPerf() throws Exception
    {
        ...
        EnumSet<ConfigurableMonitor.Item> monitoredItems = EnumSet.of(ConfigurableMonitor.Item.CMDLINE_CPU, ConfigurableMonitor.Item.CMDLINE_MEMORY, ConfigurableMonitor.Item.CMDLINE_NETWORK, ConfigurableMonitor.Item.APROF_CPU, ConfigurableMonitor.Item.JHICCUP);
        ...
        try (Cluster cluster = new Cluster(cfg))
        {
            ...
            serverArray.executeOnAll(tools ->
            {
                ...
                ServerConnector serverConnector = new ServerConnector(server, 4, 32, ssl, http);
...
//                server.setHandler(context);
            ...
            NodeArrayFuture loadersFuture = loadersArray.executeOnAll(tools ->
            {
                try (ConfigurableMonitor m = new ConfigurableMonitor(monitoredItems))
                {
                    ...
                    tools.barrier("run-start-barrier", participantCount).await();
                    runLoadGenerator(serverUri, RUN_DURATION, "loader.hlog", "status.txt", 100_000, RUN_DURATION.toSeconds() / 2);
