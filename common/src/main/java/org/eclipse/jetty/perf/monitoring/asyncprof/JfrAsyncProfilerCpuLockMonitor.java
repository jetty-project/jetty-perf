...
public class JfrAsyncProfilerCpuLockMonitor implements Monitor
{
    private final AsyncProfiler profiler;
    private final Path outputPath;

    public JfrAsyncProfilerCpuLockMonitor() throws Exception
    {
        ...
        profiler.execute("start,jfr,features=comptask,event=cpu,lock,file=" + outputPath.toAbsolutePath());
    }
    ...
    public void close() throws Exception
    {
        ...
        args.lock = true;
        JfrToHeatmap.convert(outputPath.toString(), outputPath.getParent().resolve("heatmap-lock.html").toString(), args);
        args.title = "Locks";
        JfrToFlame.convert(outputPath.toString(), outputPath.getParent().resolve("lock.html").toString(), args);
    }
}
