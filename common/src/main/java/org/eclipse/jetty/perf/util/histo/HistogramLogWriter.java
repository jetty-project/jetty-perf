 ...
 * This class was forked for one tiny addition: the ensureBufferCapacity() method
 *
 * @author Gil Tene
 */
package org.eclipse.jetty.perf.util.histo;
...
public class HistogramLogWriter
{
    ...
    public synchronized void ensureBufferCapacity(int neededByteBufferCapacity)
    {
        if ((targetBuffer == null) || targetBuffer.capacity() < neededByteBufferCapacity)
        {
            targetBuffer = ByteBuffer.allocate(neededByteBufferCapacity).order(BIG_ENDIAN);
        }
    }
