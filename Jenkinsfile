          ...
          dir("jetty.build") {
            ...
            timeout(time: 30, unit: 'MINUTES') {
              withEnv(["JAVA_HOME=${tool "jdk11"}",
                ...
                configFileProvider(
                        [configFile(fileId: 'oss-settings.xml', variable: 'GLOBAL_MVN_SETTINGS')]) {
                  sh "mvn -Pfast -ntp -s $GLOBAL_MVN_SETTINGS -V -B -U -Psnapshot-repositories -am clean install -DskipTests -T6 -e"
                }
              }
            }
          }
      ...
      steps {
        lock('jetty-perf') {
          ...
          withEnv(["JAVA_HOME=${tool "jdk11"}",
            ...
            configFileProvider(
                    [configFile(fileId: 'oss-settings.xml', variable: 'GLOBAL_MVN_SETTINGS')]) {
              sh "mvn -ntp -DtrimStackTrace=false -U -s $GLOBAL_MVN_SETTINGS -V -B -e clean install -Dtest=${TEST_TO_RUN} -Djetty.version=${JETTY_VERSION} -Dtest.jdk.name=${JDK_TO_USE} -Dtest.jdk.useLoom=${USE_LOOM_IF_POSSIBLE}" + buildMvnCmd()
            }
          }
        }
      }
