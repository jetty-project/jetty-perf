            ...
            steps {
                ...
                withEnv(["JAVA_HOME=${ tool "jdk11" }",
                  ...
                  configFileProvider(
                    ...
                    sh "mvn --no-transfer-progress -DtrimStackTrace=false -s $GLOBAL_MVN_SETTINGS -Dmaven.repo.local=.repository -V -B -e install -Dtest=${TEST_TO_RUN} -Dtest.jdk.name=${JDK_TO_USE} -Dtest.runFor=${RUN_FOR} -Djetty.version=${JETTY_VERSION} -Dloadgenerator.version=${LOADGENERATOR_VERSION}"
                  }
                }
            }
        }
